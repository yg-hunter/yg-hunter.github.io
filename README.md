...
## 11 [买卖股票的最佳时机Ⅲ best time to buy and sell stock](https://leetcode-cn.com/problems/best-time-to-buy-and-sell-stock-iii/)    
...
## 11.2 解题思路  
本题是买卖股票系列的第三题，跟前两题不同之处在于，本题限定只能交易两次，同样用动态规划解法，下面我们就来重点分析下动态规划。   
...
两步走      
 1. 状态定义：    
 	DP[i][j][k]，其中i表示到第i天；第二维j表示到当前天数时，交易了j次；第三维的k表示： 0表示当前手中没有股票；1表示当前手中有一股，可以卖掉，也可以不卖。      
	
 2. 状态转移方程：    
 	- DP[i][j][0]就表示到第i天，已经交易了j次，手中没有股票时，收益最大值。他可以从以下两个状态转移过来：  
		- DP[i-1][j][0];  // 前一天的时候也已经交易了j次，但是手中没有股票，这次也不买入股票时，收益的最大值     
		- DP[i-1][j][1]+prices[i]; //前一天的时候也已经交易了j次，但是前一天手中有一股，这次卖掉     
	则，DP[i][j][0]最大值取上面俩最大值。     


	- DP[i][j][1]表示到第i天，已经交易了j次，手中有一股时，收益最大值。它可以从以下俩状态转移过来：   
		- DP[i-1][j][1];  // 前一天已经交易了j次，且手中也有股票，这次也不卖掉      
		- DP[i-1][j-1][0]-prices[i]; //前一天已经交易了j-1次，但是手中没有股票了，这次买入一股     
